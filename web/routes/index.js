...
router.get('/', function(req, res, next) {
  ...
  async.parallel([
      function(callback){
        ...
        rest.get('http://search:1111/artists/search?name=' + artist).on('complete', function(data) {
          console.log('Artist ID is ' + data["id"]);
          rest.get('http://charts:2222/charts/' + data["id"]).on('complete', function(data) {
      ...
      function(callback){
          ...
          rest.get('http://search:1111/tracks/search?title=' + title + '&artist=' + artist).on('complete', function(data) {
            console.log('Title ID is ' + data["id"]);

            rest.get('http://images:8080/covers/' + data["id"]).on('complete', function(data) {
